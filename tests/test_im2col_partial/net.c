...
PI_L1 float im2col_partial_buffer[i2c_check_size];
#else
#define i2c_check_size (i2c_b_size_bw*2)
PI_L1 float im2col_partial_buffer_bw[i2c_check_size];
#endif
...
#if MOD==0
#define i2c_check_size (i2c_b_size*2)
PI_L1 fp16 im2col_partial_buffer[i2c_check_size];
#else
#define i2c_check_size (i2c_b_size_bw*2)
PI_L1 fp16 im2col_partial_buffer_bw[i2c_check_size];
#endif
...
#if MOD == 0
    for (int i = 0; i < i2c_check_size; i++) im2col_partial_buffer[i] = zero_val;
#else
    for (int i=0; i<i2c_check_size; i++)                                         im2col_partial_buffer_bw[i] = zero_val;
#endif
    temp_val = 0.1f;
...
static inline void train() {
#if DATA_BITS == 32
    struct im2col_partial_args im2col_partial_args;
#elif DATA_BITS == 16
    struct im2col_partial_args_fp16 im2col_partial_args;
#endif

    im2col_partial_args.input = &layer1_in;
    im2col_partial_args.c = &layer1_wgt;
    im2col_partial_args.output = &layer1_out;
    im2col_partial_args.Lpad = 0;
    im2col_partial_args.Rpad = 0;
    im2col_partial_args.Upad = 0;
    im2col_partial_args.Dpad = 0;
    im2col_partial_args.stride_h = HSTR;
    im2col_partial_args.stride_w = WSTR;
    ...
    im2col_partial_args.Rpad = RPAD;
    im2col_partial_args.Upad = UPAD;
    im2col_partial_args.Dpad = DPAD;
    im2col_partial_args.pBuffer = im2col_partial_buffer;
#else
    im2col_partial_args.Lpad = PAD_BW;
    im2col_partial_args.Rpad = PAD_BW;
    im2col_partial_args.Upad = PAD_BW;
    im2col_partial_args.Dpad = PAD_BW;
    im2col_partial_args.pBuffer = im2col_partial_buffer_bw;
#endif
...
#endif
        printf("%f ", im2col_partial_buffer[idx]);
...
#endif
        printf("%f ", im2col_partial_buffer_bw[idx]);
...
void net_step() {
    ...
    printf("\nHello, starting im2col_partial FP%d!\n", DATA_BITS);
    if (MOD == 0) {
